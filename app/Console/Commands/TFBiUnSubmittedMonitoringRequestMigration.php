...
class TFBiUnSubmittedMonitoringRequestMigration extends Command
{
    protected $signature = 'tetfund:bi-un-submitted-monitoring-requests-migration';    
    protected $description = 'Migrates un-submitted monitoring requests from TETFund_Iterum_Portal database to TETFund_BI-Portal submissions';
    ...
    public function handle() {
        ...
        echo "Running Un-Submitted Monitoring Requests Migration from TETFund_Iterum_Portal to Submission Request on TETFund_Beneficiary_Portal \n";
        ...
        echo ">>> Found {$monitoring_requests_count} Un-Submitted Monitoring Request Records for TETFund-BI-Portal Replication \n \n \n";
            ...
            try {
                ...
                if ($bi_portal_beneficiary != null) {
                        ...
                        if (empty($submission_request) || $submission_request == null) {
                            echo "Skipping - Un-Submitted Monitoring Request Record AIP (Parent) Request not Found - ERROR - Could not replicate \n";
                            continue;
                        }
                        ...
                        if (empty($bi_monitoring_request) || $bi_monitoring_request == null) {
                            ...
                            echo ">>> No. ". strval(intval($idx)+1) ." Un-Submitted Monitoring Request Record Created - {$iterum_monitoring_request->title} \n";
                            $successful_replicated_monitoring_request_count_created++;
                        } else {
                            echo ">>> No. ". strval(intval($idx)+1) ." Un-Submitted Monitoring Request Record Updated - {$iterum_monitoring_request->title}\n";
                        ...
                        echo ">>>>> Fetching Iterum-Portal Un-Submitted Monitoring Request Attachment Records \n";
                        ...
                        if (isset($iterum_monitoring_requests_attachable) && count($iterum_monitoring_requests_attachable) > 0) {
                            ...
                            echo ">>>>> Found ". $successful_replicated_monitoring_request_attachment_count ." Iterum-Portal Un-Submitted Monitoring Request Attachment Record(s) \n";
                            ...
                            foreach($iterum_monitoring_requests_attachable as $idx => $attachable_rec) {
                                ...
                                if (empty($bi_attachment_rec) || $bi_attachment_rec == null) {
                                    echo ">>>>> No. ". strval(intval($idx)+1) ." Un-Submitted Monitoring Request Attachment Record Created - {$iterum_attachment_rec->label}\n";
                                    ...
                                    echo ">>>>> No. ". strval(intval($idx)+1) ." Un-Submitted Monitoring Request Attachment Record Updated - {$iterum_attachment_rec->label}\n";
                            ...
                            echo ">>>> {$successful_replicated_monitoring_request_attachment_count} Attachment record(s) belonging to this un-submitted monitoring requests detected \n";
                            echo ">>>> {$successful_replicated_monitoring_request_attachment_count_created} Iterum-Portal un-submitted monitoring request attachment record(s) created \n";
                            echo ">>>> {$successful_replicated_monitoring_request_attachment_count_updated} Iterum-Portal un-submitted monitoring request attachment record(s) updated \n \n \n";

                        } else {
                            echo ">>>>> No Attachment Record Found For This Iterum-Portal Un-Submitted Monitoring Request \n \n \n";
                        }
        


                } else {
                    echo "Skipping - Un-Submitted Monitoring Request Record - ERROR - Could not replicate Monitoring Request \n";
                    if ($bi_portal_beneficiary == null) {
        ...
        echo "{$monitoring_requests_count} Iterum-Portal Un-Submitted Monitoring requests detected \n";
        echo "{$successful_replicated_monitoring_request_count} successfully processed \n";
        echo "{$successful_replicated_monitoring_request_count_created} Iterum-Portal un-submitted monitoring request records created \n";
        echo "{$successful_replicated_monitoring_request_count_updated} Iterum-Portal un-submitted monitoring request records updated \n";
        echo "Done Un-Submitted Monitoring Request Migration \n \n \n \n";

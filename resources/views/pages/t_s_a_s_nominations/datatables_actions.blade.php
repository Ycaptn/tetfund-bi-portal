...
<div class='btn-group' role="group">
    ...
    {{-- appears to all tsas committee members when they need to make their individual decisions --}}
    @if($is_desk_officer_check==1 && $is_average_committee_members_check==0 && auth()->user()->hasAnyRole(['BI-TSAS-committee-head', 'BI-TSAS-committee-member']) && !isset(request()->view_type))
        <a data-toggle="tooltip" 
            title="TSASNomination Committee Recommendation Approval Zone" 
            data-val='{{$id}}'
    ...
    @if($is_desk_officer_check==1 && $is_average_committee_members_check==0 && auth()->user()->hasRole('BI-TSAS-committee-head') && (isset(request()->view_type) && request()->view_type == 'committee_head_consideration' ))
        <a data-toggle="tooltip" 
            title="TSASNomination committee head finalizing consideration" 
            data-val='{{$id}}'
    ...
    @endif
     ...
     {{-- appears to all tsas committee members when approval is accomplished for preview--}}
    @if($is_average_committee_members_check==1 && $is_desk_officer_check_after_average_committee_members_checked==0 && auth()->user()->hasAnyRole(['BI-TSAS-committee-head', 'BI-TSAS-committee-member']))
        <a data-toggle="tooltip" 
            title="Preview TSASNomination considered but pending action by Desk-Officer" 
            data-val='{{$id}}'
    ...
    {{-- @if($is_desk_officer_check_after_average_committee_members_checked == 1 && $is_head_of_institution_check == 0 && auth()->user()->hasRole('BI-head-of-institution'))
        <a data-toggle="tooltip" 
            title="Head of Institution Approval for TSAS Nomination Request" 
            data-val='{{$id}}'
    ...
    {{-- @if($is_head_of_institution_check==1 && $head_of_institution_checked_status=='approved' && $is_set_for_final_submission==0 && auth()->user()->hasAnyRole(['BI-head-of-institution', 'BI-desk-officer']))
        <a data-toggle="tooltip" 
            title="Preview HOI TSASNomination approved but pending action by Desk-Officer" 
            data-val='{{$id}}'
    ...
    @if(auth()->user()->hasAnyRole(['BI-head-of-institution', 'BI-desk-officer', 'BI-astd-desk-officer']))
        <a data-toggle="tooltip" 
            title="Preview TSAS Nomination request details" 
            data-val='{{$id}}'

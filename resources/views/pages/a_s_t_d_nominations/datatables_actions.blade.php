...
<div class='btn-group' role="group">
    ...
    @if($is_desk_officer_check==1 && $is_average_committee_members_check==0 && auth()->user()->hasAnyRole(['bi-astd-committee-member']))
        <a data-toggle="tooltip" 
    ...
    @if($is_desk_officer_check==1 && $is_average_committee_members_check==0 && auth()->user()->hasRole('bi-astd-committee-head'))
        <a data-toggle="tooltip" 
            title="ASTDNomination committee head finalizing consideration" 
            data-val='{{$id}}'
    ...
    @if($is_average_committee_members_check==1 && $is_desk_officer_check_after_average_committee_members_checked==0 && auth()->user()->hasAnyRole(['bi-astd-committee-head', 'bi-astd-committee-member']))
        <a data-toggle="tooltip" 
            title="Preview ASTDNomination considered but pending action by Desk-Officer" 
            data-val='{{$id}}'
    ...
    @if($is_average_committee_members_check==1 && $is_desk_officer_check_after_average_committee_members_checked==0 && auth()->user()->hasAnyRole(['bi-desk-officer']))
        <a data-toggle="tooltip" 
            title="Preview and forward ASTDNomination details to Head of Institution" 
            data-val='{{$id}}'
    ...
    @if($is_head_of_institution_check==1 && $head_of_institution_checked_status=='approved' && $is_set_for_final_submission==0 && auth()->user()->hasRole('bi-head-of-institution'))
        <a data-toggle="tooltip" 
            title="Preview HOI ASTDNomination approved but pending action by Desk-Officer" 
            data-val='{{$id}}'
    ...
    @if($is_head_of_institution_check==1 && $head_of_institution_checked_status=='approved' && $is_set_for_final_submission==0 && auth()->user()->hasAnyRole(['bi-desk-officer']))
        <a data-toggle="tooltip" 
            title="Preview and bind ASTDNomination details to a prepared Tetfund Submission" 
            data-val='{{$id}}'
    ...
    @if($is_head_of_institution_check==1 && $is_set_for_final_submission==1 && auth()->user()->hasAnyRole(['bi-desk-officer']))
        <a data-toggle="tooltip" 
            title="Preview and alter binding submission for selected ASTDNomination details" 
            data-val='{{$id}}'
            class="btn-show-preview-binding text-primary" href="#">
